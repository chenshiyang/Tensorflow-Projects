...
def user_data_processing():
    ...
    users = users.filter(regex='UserID|Gender|Age|JobID')  # 滤除 zip-code列
...
def movie_data_processing(title_length=16):
    ...
    title2int = {val: ii for ii, val in enumerate(title_set)}  # length:5173
...
def get_feature():
    ...
    feature_pd, targets_pd = data.drop(target_fields, axis=1), data[target_fields]
    ...
    title_vocb_num = len(title_set) + 1  # 5174
...
if __name__ == '__main__':
    with timer("Preprocess movie data"):
        get_feature()
