...
def get_user_embedding(uid, user_gender, user_age, user_job):
    ...
    with tf.name_scope('user_embedding'):
        uid_embed_metrix = tf.Variable(tf.random_uniform([ #6040 * 64
            uid_max, embed_dim], -1, 1), name='uid_embed_metrix')
        uid_embed_layer = tf.nn.embedding_lookup(  # Null * 64
...
def get_user_feature_layer(
    ...
    '''
    对于输入的用户特征的embedding vector，首先分别对每个feature embedding构建一个小型神经网络
